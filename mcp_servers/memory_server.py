...
async def get_memory_stats() -> Dict[str, Any]:
    ...
    with get_db_connection() as conn:
        ...
        # Total count
        cursor.execute("SELECT COUNT(*) as count FROM memories WHERE (ttl IS NULL OR ttl > ?)", (time.time(),))
        ...
        # Get all unique tags
        cursor.execute("SELECT DISTINCT tags FROM memories WHERE (ttl IS NULL OR ttl > ?)", (time.time(),))
        all_tags = set()
        for row in cursor.fetchall():
            if row["tags"]:
...
@mcp.tool()
async def prune_old_memories(days: int = 30, exclude_types: Optional[List[str]] = None) -> Dict[str, Any]:
    """
