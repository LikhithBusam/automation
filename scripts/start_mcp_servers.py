...
def calculate_restart_delay(
    restart_count: int, base_delay: float = 1.0, max_delay: float = 60.0
) -> float:
...
def start_server_process(server: ServerState) -> bool:
    ...
    if is_port_in_use(server.port):
        ...
        log_message(
            server.name, f"Port {server.port} already in use - another server is running", "ERROR"
        )
...
async def restart_server_with_backoff(server: ServerState):
    ...
    delay = calculate_restart_delay(
        server.restart_count, server.base_restart_delay, server.max_restart_delay
    )
...
def render_simple_dashboard() -> str:
    ...
    lines.extend(
        ["-" * 70, f"Logs: {LOG_DIR.absolute()}", "Press Ctrl+C to stop all servers", "=" * 70]
    )
