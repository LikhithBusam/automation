...
class SlackMCPTool(BaseMCPTool):
    ...
    def __init__(self, server_url: str, config: Dict[str, Any]):
        super().__init__(name="slack", server_url=server_url, config=config)
    ...
    async def _send_notification(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        formatted = self._format_notification(
            notification_type, params.get("title"), params.get("message"), params.get("details", {})
        )

        return await self._send_message({"channel": params.get("channel"), "blocks": formatted})
    ...
    async def _create_thread(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        parent = await self._send_message(
            {"channel": params.get("channel"), "text": params.get("parent_text")}
        )
        ...
        for reply_text in params.get("replies", []):
            reply = await self._send_message(
                {"channel": params.get("channel"), "text": reply_text, "thread_ts": thread_ts}
            )
            replies.append(reply)

        return {"thread_ts": thread_ts, "parent": parent, "replies": replies}
    ...
    def _format_notification(
        self, notification_type: str, title: str, message: str, details: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        """Format notification blocks"""

        # Color based on type
        colors = {"success": "#36a64f", "error": "#dc3545", "warning": "#ffc107", "info": "#17a2b8"}
