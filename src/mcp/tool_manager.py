...
AGENT_PERMISSIONS: Dict[str, AgentPermissions] = {
    ...
    "project_manager": AgentPermissions(
        allowed_tools={"*"}, allowed_operations={}  # Full tool access  # No operation restrictions
    ),
}
...
class ToolRegistry:
    ...
    def register(self, name: str, tool_class: type, metadata: Dict[str, Any] = None):
...
class MCPToolManager:
    ...
    async def initialize(self):
        ...
        async with self._initialization_lock:
            ...
            for tool_name, tool_config in mcp_config.items():
                ...
                try:
                    tool = tool_class(
                        server_url=tool_config.get(
                            "server_url", f"http://localhost:{3000 + len(self.tools)}"
                        ),
                        config=tool_config,
                    )
                    await tool.connect()
        ...
        if mcp_config.get("github", {}).get("enabled", False):
            try:
                ...
                self.tools["github"] = GitHubMCPTool(
                    server_url=mcp_config["github"]["server_url"], config=mcp_config["github"]
                )
        ...
        if mcp_config.get("memory", {}).get("enabled", False):
            try:
                ...
                self.tools["memory"] = MemoryMCPTool(
                    server_url=mcp_config["memory"]["server_url"], config=mcp_config["memory"]
                )
        ...
        if mcp_config.get("slack", {}).get("enabled", False):
            try:
                ...
                self.tools["slack"] = SlackMCPTool(
                    server_url=mcp_config["slack"]["server_url"], config=mcp_config["slack"]
                )
    ...
    async def _handle_error(
        self, tool_name: str, operation: str, params: Dict[str, Any], error: Exception
    ) -> Optional[Any]:
    ...
    def register_error_handler(self, tool_name: str, operation: str, handler: callable):
    ...
    def get_tools_for_agent(self, agent_name: str, tool_names: List[str] = None) -> List[Any]:
        ...
        if tool_names:
            ...
            requested_tools = (
                perms.allowed_tools if "*" not in perms.allowed_tools else set(self.tools.keys())
            )
    ...
    def _get_tool_operations(self, tool_name: str) -> Dict[str, Dict[str, Any]]:
        ...
        operations = {
            "github": {
                ...
                "get_commit": {"description": "Get commit details", "params": ["repo", "sha"]},
                "trigger_deployment": {
            ...
            "filesystem": {
                "read_file": {"description": "Read the contents of a file", "params": ["path"]},
                "write_file": {
            ...
            "memory": {
                ...
                "delete": {"description": "Delete a memory", "params": ["id"]},
                "prune": {
                    "description": "Prune old or low-value memories",
                    "params": ["max_age_days", "max_count"],
                },
                "get_stats": {"description": "Get memory statistics", "params": []},
            },
            "slack": {
                "send_message": {
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        """
        health_status = {"timestamp": datetime.now().isoformat(), "tools": {}, "overall": "healthy"}
        ...
        for tool_name, tool in self.tools.items():
            try:
                ...
                health_status["tools"][tool_name] = {"status": "unhealthy", "error": str(e)}
                unhealthy_count += 1

        if unhealthy_count > 0:
            health_status["overall"] = (
                "degraded" if unhealthy_count < len(self.tools) else "unhealthy"
            )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        if total > 0:
            stats["aggregated"][
                "success_rate"
            ] = f"{(stats['aggregated']['successful_calls'] / total * 100):.1f}%"

        cache_total = stats["aggregated"]["cache_hits"] + stats["aggregated"]["cache_misses"]
        if cache_total > 0:
            stats["aggregated"][
                "cache_hit_rate"
            ] = f"{(stats['aggregated']['cache_hits'] / cache_total * 100):.1f}%"
