...
class MemoryManager:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.tier_ttl = {
            MemoryTier.SHORT_TERM: timedelta(
                seconds=config.get(
                    "short_term_ttl", TIER_CONFIGS[MemoryTier.SHORT_TERM].ttl_seconds
                )
            ),
            MemoryTier.MEDIUM_TERM: timedelta(
                seconds=config.get(
                    "medium_term_ttl", TIER_CONFIGS[MemoryTier.MEDIUM_TERM].ttl_seconds
                )
            ),
    ...
    def _init_mcp_memory(self, config: Dict[str, Any]):
        ...
        try:
            ...
            self.mcp_memory = MemoryMCPTool(
                server_url=mcp_config.get("server_url", "http://localhost:8002"), config=mcp_config
            )
        ...
        **kwargs,
        ...
        try:
            ...
            if tier == MemoryTier.SHORT_TERM:
                evicted = self.short_term_cache.put(
                    entry_id,
                    {
                        "id": entry_id,
                        "content": content,
            ...
            self.stats.type_counts[type.value if isinstance(type, MemoryType) else type] = (
                self.stats.type_counts.get(type.value if isinstance(type, MemoryType) else type, 0)
                + 1
            )
            if agent:
    ...
    async def promote(self, entry_id: str, new_tier: MemoryTier) -> bool:
        ...
        """
        try:
            entry = self.db_session.query(MemoryEntry).filter(MemoryEntry.id == entry_id).first()
            ...
            if new_idx <= current_idx:
                self.logger.warning(
                    f"Cannot demote or same-tier promote: {current_tier} -> {new_tier}"
                )
                return False
            ...
            self.stats.tier_counts[current_tier.value] = max(
                0, self.stats.tier_counts.get(current_tier.value, 1) - 1
            )
    ...
    async def consolidate(
        self, tier: Optional[MemoryTier] = None, similarity_threshold: Optional[float] = None
    ) -> int:
        """
        ...
        try:
            ...
            for merged_id in merge_map:
                self.db_session.query(MemoryEntry).filter(MemoryEntry.id == merged_id).delete()
    ...
    async def get_context(
        ...
        try:
            ...
            short_term = await self.retrieve(
                query=query, tier=MemoryTier.SHORT_TERM, agent=agent_id, limit=5
            )
            context["short_term"] = short_term

            # Get medium-term memories (project knowledge)
            medium_term = await self.retrieve(
                query=query, tier=MemoryTier.MEDIUM_TERM, agent=agent_id, limit=10
            )
            context["medium_term"] = medium_term

            # Get long-term memories (permanent knowledge)
            long_term = await self.retrieve(query=query, tier=MemoryTier.LONG_TERM, limit=10)
            context["long_term"] = long_term

            # Get recent decisions
            decisions = await self.retrieve(
                query=query, type=MemoryType.DECISION, agent=agent_id, limit=5
            )
            context["recent_decisions"] = decisions

            # Get patterns
            patterns = await self.retrieve(query=query, type=MemoryType.PATTERN, limit=5)
            context["patterns"] = patterns

            # Calculate totals
            context["total_memories"] = (
                len(short_term) + len(medium_term) + len(long_term) + len(decisions) + len(patterns)
            )
    ...
    async def check_and_promote(self) -> int:
        ...
        try:
            ...
            short_candidates = (
                self.db_session.query(MemoryEntry)
                .filter(
    ...
    async def evict_lru(self, count: int = 10) -> int:
        ...
        try:
            ...
            entries = (
                self.db_session.query(MemoryEntry)
                .filter(MemoryEntry.tier == MemoryTier.SHORT_TERM.value)
                .order_by(MemoryEntry.last_accessed_at.asc())
                .limit(count)
                .all()
            )
    ...
    def _trim_context_to_tokens(self, context: Dict[str, Any], max_tokens: int) -> Dict[str, Any]:
        """
    ...
    async def get_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for tier in MemoryTier:
                count = (
                    self.db_session.query(MemoryEntry)
                    .filter(MemoryEntry.tier == tier.value)
                    .count()
                )
            ...
            for mem_type in MemoryType:
                count = (
                    self.db_session.query(MemoryEntry)
                    .filter(MemoryEntry.type == mem_type.value)
                    .count()
                )
    ...
    async def cleanup_expired(self) -> int:
        ...
        try:
            ...
            deleted = (
                self.db_session.query(MemoryEntry)
                .filter(MemoryEntry.ttl.isnot(None), MemoryEntry.ttl < now)
                .delete()
            )
    ...
    async def _persist_to_mcp(self, entry_id: str, content: str, tier: MemoryTier, tags: List[str]):
        ...
        if self.redis_client:
            try:
                await self.redis_client.setex(
                    f"memory:entry:{entry_id}", 3600, json.dumps(data, default=str)  # 1 hour
                )
            except Exception as e:
    ...
    async def search(
        ...
        return await self.retrieve(
            query=query, tier=layer, agent=agent, limit=limit, min_relevance=min_relevance
        )
    ...
    async def update(self, entry_id: str, updates: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            for key, value in updates.items():
                if key == "content":
                    entry.content = value
                    entry.embedding = json.dumps(self._create_embedding(value).tolist())
                    if len(value) > 500:
