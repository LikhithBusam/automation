...
class ModelFactory:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.hf_token = config.get("huggingface", {}).get("api_token") or os.environ.get("HUGGINGFACE_TOKEN")
    ...
    def get_gemini_llm(self, agent_type: str = "default"):
        ...
        config = gemini_configs.get(agent_type, {
    ...
    def get_groq_llm(self, agent_type: str = "default"):
        ...
        config = groq_configs.get(agent_type, {
        ...
        **kwargs
        ...
        if use_cache:
            cached_response = self.prompt_cache.get(
                prompt, loaded_model.model_name, cache_params
            )
            if cached_response:
        ...
        try:
            ...
            if loaded_model.deployment_type == DeploymentType.LOCAL:
                response, token_usage = self._generate_local(
                    loaded_model, prompt, max_tokens, temperature,
            ...
            if use_cache and not stream:
                self.prompt_cache.put(
                    prompt, loaded_model.model_name, cache_params, response
                )
            ...
            if loaded_model.deployment_type == DeploymentType.LOCAL and self.inference_client:
                ...
                return self._generate_api_fallback(
                    loaded_model.model_name, prompt, max_tokens,
    ...
    def _generate_api(
        ...
        completion_tokens = response.details.generated_tokens if hasattr(response, 'details') else len(response.generated_text.split()) * 1.3
    ...
    def _ensure_model_capacity(self):
        ...
        while len(self.loaded_models) >= self.max_loaded_models:
            ...
            lru_key = min(
                ...
                key=lambda k: self.loaded_models[k].last_used_at
            )
