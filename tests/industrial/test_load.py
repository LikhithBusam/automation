...
class LoadTester:
    ...
    async def run_concurrency_test(
        ...
        for num_users in concurrent_users:
            ...
            results.append(ConcurrencyTestResult(
...
class TestLoadPerformance:
    ...
    async def test_high_throughput(self, load_tester):
        ...
        assert result.p99_response_time_ms < 500, \
    ...
    async def test_memory_stability(self, load_tester):
        ...
        assert result.memory_delta_mb < 50, \
            f"Memory grew too much: {result.memory_delta_mb}MB"
    ...
    async def test_error_resilience(self, load_tester):
        ...
        assert 0.85 <= success_rate <= 0.95, \
            f"Unexpected success rate: {success_rate:.2%}"
...
class TestConnectionPoolLoad:
    ...
    async def test_pool_reuses_connections(self):
        ...
        assert len(connections_used) <= 5, \
            f"Created too many connections: {len(connections_used)}"
