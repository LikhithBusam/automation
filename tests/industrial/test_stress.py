...
class TestConcurrencyStress:
    ...
    async def test_extreme_concurrency(self):
        ...
        async def extreme_operation():
            nonlocal successes
            try:
                conn = await asyncio.wait_for(pool.get_connection("stress_test"), timeout=5.0)
...
class TestTimeoutHandling:
    ...
    async def test_partial_results_on_timeout(self):
        ...
        tasks = [asyncio.create_task(fast_task(i)) for i in range(5)] + [
...
class TestResourceExhaustion:
    ...
    async def test_connection_pool_exhaustion(self):
        ...
        try:
            conn = await asyncio.wait_for(pool.get_connection("exhaust_test"), timeout=0.5)
    ...
    async def test_handles_many_concurrent_waits(self):
        ...
        async def wait_and_use():
            ...
            try:
                conn = await asyncio.wait_for(pool.get_connection("wait_test"), timeout=2.0)
                await asyncio.sleep(0.01)
                await pool.release_connection(conn)
                completed += 1
            except asyncio.TimeoutError:
