...
class ProductionChecker:
    def __init__(self):
        self.results = {"critical": [], "warnings": [], "passed": [], "info": []}
...
def main():
    ...
    for name, pid_file in server_pids:
        if Path(pid_file).exists():
            ...
            checker.warning(
                f"{name}", False, "Not running - start with scripts/start_mcp_servers.py"
            )
    ...
    checker.info(
        "SQLite Database", f"{'Exists' if sqlite_db.exists() else 'Will be created on first use'}"
    )

    fallback = Path("data/memory_fallback.json")
    checker.info("Fallback Storage", f"{'Active' if fallback.exists() else 'Ready to create'}")
    ...
    if gitignore:
        ...
        for pattern, desc in critical_patterns:
            ...
            checker.critical(f"Excludes {desc}", in_gitignore, "Prevents sensitive data in git")
        ...
        if Path(sf).exists():
            ...
            try:
                result = subprocess.run(
                    ["git", "ls-files", "--error-unmatch", sf], capture_output=True, text=True
                )
    ...
    if exposed_files:
        checker.critical("No sensitive files in git", False, f"EXPOSED: {', '.join(exposed_files)}")
    else:
    ...
    if len(checker.results["critical"]) == 0:
        ...
        print(
            f"   Status: BLOCKED - {len(checker.results['critical'])} critical issue(s) must be fixed"
        )
